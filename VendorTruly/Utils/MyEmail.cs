namespace VendorTruly.Utils
{
    public class MyEmail
    {
        public static bool SendValidateCode(string code, string emailAddress, string username)
        {
            ...
            //直接调用封装在dll的邮件发送函数
            return TrulyEmail.EmailUtil.SemiSend("信利事业部送货平台", subject, content, emailAddress, "", false);
        ...
        public static bool SendEmail(string subject, string emailAddrs, string content, string ccEmailAddrs = "")
        {
            if (!string.IsNullOrEmpty(emailAddrs)) {
                return TrulyEmail.EmailUtil.SemiSend("信利事业部送货平台", subject, content, emailAddrs, ccEmailAddrs,false);
            }
            return true;
        }
    }
